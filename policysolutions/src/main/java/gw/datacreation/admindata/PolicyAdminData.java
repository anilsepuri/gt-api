...
import gw.datacreation.APIUtil;
...
public class PolicyAdminData {
    ...
    private final static String dataOutputFileName = "/policyDataContainer.json";
    private static String dir = "";
    // outputDir can be different based on from where this class is getting called. eg. policysolutions / multipapsolutions
    ...
    public static void loadAdminData(String moduleName) {
        ...
        //Read saved data
        JSONObject data = readJSONFile(outputDir + dataOutputFileName);
        ...
        if(!data.isEmpty()) {
            ...
            ((JSONArray)attributesObj.get("producerCodes")).stream().forEach(element -> {
                ...
                PolicyDataContainer.setPolicyProducerCode(obj.get("producerCodeIdentifier").toString(),
                        new ProducerCodeDO(obj.get("producerCode").toString() ,
                                ...
                                obj.get("organizationID").toString()));
            });
            ...
            ((JSONArray)attributesObj.get("groups")).stream().forEach(element -> {
                ...
                PolicyDataContainer.setPolicyGroup(obj.get("groupIdentifier").toString(),
                        new GroupDO(obj.get("groupName").toString() ,
            ...
            //User data not found in JSON file; create new users
    ...
    public static JSONObject createDataFromInputJSONFile() {
        //read json file
        JSONObject inputObj = readJSONFile(dir + dataInputFileName);
        ...
        return saveDataToFile(producerCodes, groups, users);
    }
    ...
    public static JSONArray generateProducerCodes(JSONArray producerCodes) {
        ...
        producerCodes.stream().forEach(e -> {
            ...
            String producerCode = getRandomName("producerCode");

            //Generate Producer Code
            HashMap<String,Object> result = (HashMap) APIUtil.executeGtApiFeature(
                    PRODUCER_CODES_FEATURE,
                    new HashMap<>(){{
                        put("code", producerCode);
                        put("organization", element.get("organization"));
                        put("roles", "producer");
                    }}).get("response");

            //Add in PolicyDataContainer
            PolicyDataContainer.setPolicyProducerCode(element.get("producerCodeIdentifier").toString(),
                    new ProducerCodeDO(result.get("code").toString(),
                            result.get("id").toString(),
                            result.get("organization").toString()));
            ...
            pCodes.add( new JSONObject(){{
                ...
                put("organizationID", result.get("organization"));
            }});
        });
        return pCodes;
    }
    ...
    public static JSONArray generateGroups(JSONArray array) {
        ...
        array.stream().forEach(e -> {
            JSONObject element = (JSONObject)e;
            String groupName = getRandomName("group");
            //Generate Groups
            Map<String,Object> result = (HashMap) APIUtil.executeGtApiFeature(
                    GROUPS_FEATURE,
                        new HashMap<>(){{
                            put("groupName", groupName);
                            put("organization", PolicyDataContainer.getPolicyProducerCode(element.get("producerCodeIdentifier").toString()).getOrganization());
    ...
    public static JSONArray generateUsers(JSONArray array) {
        ...
        array.stream().forEach(e -> {
            JSONObject element = (JSONObject)e;
            String userName = getRandomName("user");
            ...
            Map<String,Object> result = (HashMap) APIUtil.executeGtApiFeature(
                    ...
                    new HashMap<>(){{
                        ...
                        put("uWAuthorityProfiles", ((JSONArray)(element.get("uWAuthorityProfiles"))).get(0));
                    }}).get("response");
    ...
    public static JSONObject saveDataToFile(JSONArray pCodes, JSONArray groups, JSONArray users) {
        ...
        saveDataToFile(obj, outputDir + dataOutputFileName);
        return obj;
    }
    ...
    public static void setOutputDirPath(String moduleName) {
        String userDirectory = FileSystems.getDefault()
                ...
                .toAbsolutePath()
                .toString();
        outputDir = String.format("%s/%s",
                userDirectory.replaceFirst(moduleName,"")
                        + "/" + moduleName + "/src/main/java" ,
                PolicyAdminData.class.getPackage().getName().replace(".", "/"));
        dir = String.format("%s/%s",
                userDirectory.replaceFirst(moduleName,"")
                        + "/policysolutions/src/main/java" ,
                PolicyAdminData.class.getPackage().getName().replace(".", "/"));
    }

    public static void saveDataToFile(JSONObject obj, String dataOutputFile) {
        try {
